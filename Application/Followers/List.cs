        ...
        public class Handler : IRequestHandler<Query, List<Profile>>
        {
            ...
            public async Task<List<Profile>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                switch (request.Predicate)
                {
                    case "followers":
                        {
                            userFollowings = await queryable.Where(o => o.Target.UserName == request.Username).ToListAsync();
                    ...
                    case "following":
                        {
                            userFollowings = await queryable.Where(o => o.Observer.UserName == request.Username).ToListAsync();
